...
function buildPostHTML(p, idx) {
  ...
  return `<div class="post-card">
    ...
    <div class="post-actions">
      <button class="act-btn ${liked ? 'liked' : ''}" onclick="likePost('${p.id}',this)" title="Like">❤️ ${fmtNum(p.likes || 0)}</button>
...
window.switchTab = function (btn, page) {
  ...
  const allSections = ['pg-projects', 'pg-events', 'pg-saved', 'pg-myposts', 'pg-trends'];
  ...
  if (page === 'feed') {
    ...
    if (page === 'myposts') {
      viewingUserId = null; // Revert to self when clicking the tab directly
      renderMyPosts();
    }
