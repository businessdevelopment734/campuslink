...
function initUI() {
  ...
  if (!deepLinkPostId) {
    ...
    // If deep link exists, ensure we are on the Feed tab
    const feedTabBtn = document.querySelector('.ptab[onclick*="feed"]');
    if (feedTabBtn) switchTab(feedTabBtn, 'feed');
  }
      ...
      requestAnimationFrame(() => {
        ...
        if (Math.abs(delta) >= THRESHOLD) {
          if (delta > 0 && currentY > TOP_GUARD) {
            ...
            pageTabs?.classList.add('tabs-hidden');
            mobileNav?.classList.add('mob-hidden');
          } else {
            // ── Scrolling UP or near top → reveal ──
            navbar?.classList.remove('nav-hidden');
            pageTabs?.classList.remove('tabs-hidden');
...
function handleDeepLinkActions() {
  const urlParams = new URLSearchParams(window.location.search);
  const postId = urlParams.get('post') || window.location.hash.replace('#post-', '');
  if (postId && !window._deepLinkedHandled) {
    ...
    if (postEl) {
      ...
      // Clean up URL without reload to prevent re-scroll on refresh
...
function listenNotifications() {
  ...
  onSnapshot(q, snap => {
    ...
    // 1. General notifications (excluding messages)
    ...
    const msgNotifs = all.filter(n => n.type === 'message');
    const unreadMsgs = msgNotifs.filter(n => !n.read).length;
