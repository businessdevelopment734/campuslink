...
let viewingUserId = null; // null means viewing own profile
let unreadMessageIds = []; // Track IDs to clear them easily
let unreadNotifIds = [];   // Track general notification IDs
...
function initUI() {
  ...
  const savedTheme = userProfile.theme || localStorage.getItem('cl_theme') || 'classic';
  ...
  // ── Switch to Events tab as the default landing view (if no deep link) ──
  const urlParams = new URLSearchParams(window.location.search);
  const deepLinkPostId = urlParams.get('post') || window.location.hash.replace('#post-', '');
  const userId = urlParams.get('u');

  if (!deepLinkPostId && !userId) {
    const eventsTabBtn = document.querySelector('.ptab[onclick*="events"]');
    if (eventsTabBtn) switchTab(eventsTabBtn, 'events');
  } else {
    // If deep link exists, ensure we are on the appropriate tab
    const targetTab = userId ? 'myposts' : 'feed';
    const tabBtn = document.querySelector(`.ptab[onclick*="${targetTab}"]`);
    if (tabBtn) switchTab(tabBtn, targetTab);
  }

  // Also sync sidebar & mobile nav to Events
  ...
  // ── Smart scroll: hide header & bottom nav on scroll-down, reveal on scroll-up ──
  initScrollHide();
}
...
function initScrollHide() {
  ...
  let lastY = 0;
  let ticking = false;
  const THRESHOLD = 8;   // minimum px moved before toggling
  const TOP_GUARD = 72;  // never hide when within 72px of top
      ...
      requestAnimationFrame(() => {
        ...
        if (Math.abs(delta) >= THRESHOLD) {
          if (delta > 0 && currentY > TOP_GUARD) {
            // ── Scrolling DOWN → hide navbar, keep tabs ──
            mainHeader?.classList.add('nav-hidden');
            mobileNav?.classList.add('mob-hidden');
          } else {
            // ── Scrolling UP or near top → reveal ──
            mainHeader?.classList.remove('nav-hidden');
            mobileNav?.classList.remove('mob-hidden');
          }
          lastY = currentY;
        }

        // Stronger shadow when user has scrolled away from top
...
window.openCalendar = function () {
  ...
  renderCalendar();
  renderDateEvents(selectedCalendarDate);
};
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (hasEventsOnDate(date)) dayDiv.classList.add('has-event');
...
/**
 * 📢 CLIENT-SIDE MASS BROADCAST: Fetches subscribed users and sends a single email using BCC.
 * 💡 IMPORTANT: On your EmailJS dashboard, edit your template and set the "BCC" field to {{bcc_list}}
 */
async function notifySubscribersEmailJS(eData, description, imageURL) {
  try {
    ...
    // Collect all emails in a comma-separated list
    const emailList = snap.docs.map(d => d.data().email).filter(Boolean).join(',');

    if (!emailList) return;

    console.log(`📧 Sending BCC broadcast to ${snap.docs.length} users with EmailJS...`);
    ...
    // Send a single request with everyone in BCC
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams);
    console.log('✅ Mass notification sent successfully!');

  } catch (e) {
    console.error('Email notification broadcast failed:', e);
  }
}
...
function renderDateEvents(date) {
  ...
  label.textContent = `Events on ${dateStr}`;

  const dayStr = date.toDateString();
  const filtered = posts.filter(p => {
    if (!p.eventData || !p.eventData.date) return false;
    const evDate = new Date(p.eventData.date);
    return evDate.toDateString() === dayStr;
  });
  ...
  listContainer.innerHTML = filtered.map((p, i) => {
    ...
    return `
      <div class="cal-event-card" style="animation-delay: ${i * 0.1}s" onclick="closeCalendar(); switchToFeedAndScroll('${p.id}')">
        ...
        <div class="cec-info">
          <h5>${escHTML(p.eventData.title || 'Untitled Event')}</h5>
          <p>${escHTML(p.text || '').slice(0, 80)}...</p>
          <div class="cec-footer">
            <span class="cec-time">⏰ ${evTime}</span>
            <span class="cec-type">${typeBadge(p.type)}</span>
          </div>
        </div>
      </div>`;
  }).join('');
...
function listenPosts() {
  ...
  unsubPosts = onSnapshot(q, snap => {
    ...
    renderPosts();
    // Re-render the Events page whenever posts change so date-groups stay fresh
    const mypContainer = document.getElementById('myPostsContainer');
    if (mypContainer && mypContainer.offsetParent !== null) renderMyPosts();

    const challContainer = document.getElementById('pg-challenges');
    if (challContainer && challContainer.offsetParent !== null) renderChallenges();
  ...
  return `<div class="post-card">
    <div class="post-header">
      ...
      <div class="post-header-right">
        ...
        ${isOwnPost || (userProfile?.isAdmin || userProfile?.role === 'admin') ? `<button class="icon-btn" style="width:34px; height:34px; font-size:1rem; border-color:var(--border); background:var(--bg2)" onclick="openEditModal('${p.id}')" title="Edit Post">✏️</button>` : ''}
        ${isOwnPost || (userProfile?.isAdmin || userProfile?.role === 'admin') ? `<button class="icon-btn" style="width:34px; height:34px; font-size:1rem; border-color:var(--border); background:var(--bg2); color:#ef4444" onclick="deletePost('${p.id}')" title="Delete Post">🗑️</button>` : ''}
...
function typeBadge(type) {
  const map = { text: '📝 Post', image: '🖼️ Image', link: '🔗 Link', project: '🛠️ Project', hackathon: '🏆 Hackathon', workshop: '⚙️ Workshop', cultural: '🎨 Cultural', symposium: '🏛️ Symposium' };
...
function listenNotifications() {
  ...
  onSnapshot(qList, snap => {
    ...
    if (notifDropOpen) renderNotifDropdown(notifs);
  });
  ...
  onSnapshot(qUnread, snap => {
    // We get both the data and the ID to allow for easy clearing
    const unreadRaw = snap.docs.map(d => ({ id: d.id, ...d.data() }));

    // Track IDs specifically for the dropdown clearing
    ...
    if (nBadge) {
      nBadge.textContent = unreadNotifs || '';
      nBadge.style.display = unreadNotifs ? 'flex' : 'none';
    }

    // Message notifications badge
    const unreadMsgs = unreadMessageIds.length;
    const mBadge = document.querySelector('.msg-badge');
    if (mBadge) {
      mBadge.textContent = unreadMsgs || '';
      mBadge.style.display = unreadMsgs ? 'flex' : 'none';
    }
  });
}
...
function listenMessages() {
  ...
  onSnapshot(q, snap => {
    ...
    // Badge is now handled by listenNotifications
    // based on 'message' type notification items.
...
function renderEvents() {
  ...
  // Today boundaries (midnight → end of day)
  const now = new Date();
...
function buildEventSection(title, events, type) {
  if (!events.length) {
    return `
      <div class="ev-section ev-section-${type}">
...
function buildEventCard(p, idx, sectionType) {
  ...
  const titleBadge = typeBadge(p.type);
  ...
  return `
    <div class="ev-card ${isPast ? 'ev-card-past' : ''} ${isToday ? 'ev-card-today' : ''}"
      ...
      ? `<div class="ev-card-img-wrap">
             <img src="${p.imageURL}" alt="${escHTML(p.eventData.title || '')}" class="ev-card-img" loading="lazy" onerror="this.parentElement.style.display='none'">
      ...
      <div class="ev-card-body">
        <h3 class="ev-card-title">${escHTML(p.eventData.title || 'Untitled Event')}</h3>

        ${college ? `<p class="ev-card-college">🏫 ${escHTML(college)}</p>` : ''}
        ...
        ${desc ? `<p class="ev-card-desc">${escHTML(desc)}${p.text.length > 120 ? '…' : ''}</p>` : ''}

        <div class="ev-card-footer">
          <span class="ev-author">
            <span class="ev-author-avatar">${getInitials(p.authorName)}</span>
            ${escHTML(p.authorName || 'User')}
          </span>
...
window.createPost = async function () {
  ...
  const eventCollege = document.getElementById('df-event-group').style.display !== 'none' ? document.getElementById('postEventCollege').value.trim() : '';
  ...
  try {
    ...
    if (['hackathon', 'workshop', 'cultural', 'symposium'].includes(type)) {
      ...
      // Send EmailJS notifications to all subscribed users from client-side
      notifySubscribersEmailJS(eData, text, imageURL);
    }

    await addDoc(collection(db, 'posts'), postData);

    // ── CHALLENGE PROGRESS LOGIC ──
    if (activeChallengeId && activeDayNum) {
      const challRef = doc(db, 'challenges', activeChallengeId);
      await updateDoc(challRef, {
        completedDays: arrayUnion(activeDayNum)
      });
      toast(`✅ Day ${activeDayNum} Smashed! Challenge progress updated.`);
...
window.openEditModal = async function (postId) {
  try {
    ...
    if (['hackathon', 'workshop', 'cultural', 'symposium'].includes(p.type) || p.eventData) {
      ...
      if (evGroup) evGroup.style.display = 'none';
    }
...
window.sharePost = async function (postId, btn) {
  try {
    ...
    const hasShared = (post.sharedBy || []).includes(currentUser.uid);
    if (!hasShared) {
      await updateDoc(doc(db, 'posts', postId), {
        shares: increment(1),
        sharedBy: arrayUnion(currentUser.uid)
      });
...
window.switchTab = function (btn, page) {
  ...
  const allSections = ['pg-projects', 'pg-events', 'pg-saved', 'pg-myposts', 'pg-trends', 'pg-challenges'];
...
window.applyTheme = function (name, el, save = true) {
  ...
  if (save) {
    ...
    if (currentUser) {
      updateDoc(doc(db, 'users', currentUser.uid), { theme: name })
...
function buildSavedCard(post, meta) {
  ...
  return `<div class="post-card saved-card">
    ...
    <div class="post-header">
      ...
      <div class="post-header-right">
        ${(post.authorId && post.authorId !== currentUser?.uid) ? `<button class="icon-btn" style="width:34px; height:34px; font-size:1rem; border-color:var(--border); background:var(--bg2)" onclick="window.location.href='messages.html?u=${post.authorId}'" title="Message ${post.authorName}">💬</button>` : ''}
...
function renderMyPosts() {
  ...
  const isSelf = !viewingUserId || viewingUserId === currentUser.uid;
  ...
  if (isSelf) {
    ...
    getDoc(doc(db, 'users', viewingUserId)).then(snap => {
      ...
      const userPosts = posts.filter(function (p) { return p.authorId === viewingUserId; });
...
window.openChallComposer = function() {
  document.getElementById('challComposerOverlay').classList.add('open');
};
window.closeChallComposer = function(e) {
  if (e && e.target !== document.getElementById('challComposerOverlay')) return;
  document.getElementById('challComposerOverlay').classList.remove('open');
};

window.createChallenge = async function() {
  const topic = document.getElementById('challTopic').value;
  const days = parseInt(document.getElementById('challDays').value) || 30;
  const start = document.getElementById('challStartDate').value;
  const end = document.getElementById('challEndDate').value;
  const desc = document.getElementById('challDesc').value;

  if (!topic) return toast('Please enter a challenge topic');
  if (!currentUser) return toast('Log in to create challenges');

  try {
    const challData = {
      authorId: currentUser.uid,
      authorName: userProfile.displayName || 'Student',
      authorAvi: userProfile.photoURL || '',
      title: topic,
      totalDays: days,
    ...
    await addDoc(collection(db, 'challenges'), challData);
    toast('🏆 Challenge started! Day 1 begins now.');
    closeChallComposer();
    renderChallenges();
  } catch (err) {
    console.error(err);
    toast('Error starting challenge');
  }
};
...
window.renderChallenges = function() {
  ...
  // We remove orderBy from the query to avoid missing index errors in Firestore.
  // We will sort the results in memory instead.
  const q = query(collection(db, 'challenges'), where('authorId', '==', currentUser.uid));
  ...
  onSnapshot(q, (snap) => {
    ...
    container.innerHTML = sortedDocs.map(doc => {
      ...
      const progress = Math.round((completedCount / (c.totalDays || 30)) * 100);
      return `
        <div class="chall-card" onclick="openChallView('${id}')">
          ...
          <div class="chall-card-meta">
             <span>${completedCount}/${c.totalDays} Days Done</span>
             <span>${progress}%</span>
          </div>
        </div>
      `;
    ...
    container.innerHTML = `<p style="color:red;padding:20px">Error loading challenges: ${err.message}</p>`;
  });
};

window.openChallView = async function(id) {
  activeChallengeId = id;
  const overlay = document.getElementById('challViewOverlay');
  overlay.classList.add('open');
  ...
  const start = c.startDate ? new Date(c.startDate).toLocaleDateString() : 'Today';
  document.getElementById('cv-meta').textContent = `${c.totalDays} Days • Started ${start}`;

  const grid = document.getElementById('cv-days-grid');
  grid.innerHTML = '';

  const completed = new Set(c.completedDays || []);
  const currentDay = (c.completedDays.length || 0) + 1;

  for (let i = 1; i <= c.totalDays; i++) {
    let mode = 'upcoming'; 
    if (completed.has(i)) mode = 'completed';
    else if (i === currentDay) mode = 'active';
    else if (i > currentDay) mode = 'disabled';

    const dayEl = document.createElement('div');
    dayEl.className = `day-circle ${mode}`;
    dayEl.innerHTML = `<span class="day-num">${i}</span><span class="day-lbl">Day</span>`;
    
    if (mode === 'active' || mode === 'completed') {
      dayEl.onclick = () => {
         if (mode === 'active') openComposerForChallenge(i);
         else toast(`Day ${i} already smashed! 🏆`);
      };
    }
...
window.closeChallView = function(e) {
  if (e && e.target !== document.getElementById('challViewOverlay')) return;
  document.getElementById('challViewOverlay').classList.remove('open');
  document.getElementById('cvFabMenu').classList.remove('open');
};

window.toggleChallFab = function() {
  document.getElementById('cvFabMenu').classList.toggle('open');
};

window.openComposerForChallenge = function(day) {
  // If no day provided, find the next active day
  if (!day) {
    const grid = document.getElementById('cv-days-grid');
    const activeDay = grid ? grid.querySelector('.day-circle.active') : null;
    day = activeDay ? parseInt(activeDay.querySelector('.day-num').textContent) : null;
  }
  
  if (!day) return toast('No active day to post for!');
  
  activeDayNum = day;
  openComposer();
  
  // Customizing the composer UI for the challenge streak
  const composerTitle = document.querySelector('#postComposerOverlay h3');
  const textarea = document.getElementById('postContent');
  
  if (composerTitle) composerTitle.textContent = `🏆 Challenge Update: Day ${day}`;
  if (textarea) textarea.placeholder = `Whats your progress for Day ${day}? Share your wins!`;
};

// State for tracking challenge-specific posting
