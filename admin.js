...
window.switchAdminTab = function(tab) {
  const views = ['dashboard', 'users', 'posts', 'messages', 'reports'];
  const titleMap = { dashboard: 'Dashboard', users: 'Users', posts: 'Posts', messages: 'Messaging Control', reports: 'Reports' };

  views.forEach(v => {
    const el = document.getElementById(`view-${v}`);
    if (el) {
      if (v === tab) {
        el.classList.remove('admin-section-hidden');
        el.style.display = 'block';
      } else {
        el.classList.add('admin-section-hidden');
  ...
  if (tab === 'users') { loadUsers(); loadEmailMasterState(); }
  if (tab === 'posts') loadPosts();
  if (tab === 'messages') { /* No pre-load needed */ }
  if (tab === 'reports') loadReports();
};

// ── EMAIL MASTER TOGGLE ───────────────────────────────────────
async function loadEmailMasterState() {
  const btn = document.getElementById('emailMasterBtn');
  const lbl = document.getElementById('emailMasterLabel');
  if (!btn || !lbl) return;
  try {
    const snap = await getDoc(doc(db, 'settings', 'email'));
    const enabled = snap.exists() ? snap.data().enabled !== false : true; // default ON
    applyEmailMasterUI(enabled);
  } catch(e) {
    console.warn('Could not load email master state:', e);
  }
}

function applyEmailMasterUI(enabled) {
  const btn = document.getElementById('emailMasterBtn');
  const lbl = document.getElementById('emailMasterLabel');
  if (!btn || !lbl) return;
  btn.classList.remove('on', 'off');
  btn.classList.add(enabled ? 'on' : 'off');
  lbl.textContent = enabled ? '📧 All Subscribed' : '🔕 All Unsubscribed';
}

window.toggleEmailMaster = async () => {
  const btn = document.getElementById('emailMasterBtn');
  ...
  const confirmMsg = newState
    ? '✅ This will SUBSCRIBE all users to emails. Continue?'
    : '🔕 This will UNSUBSCRIBE all users from emails. Continue?';
  if (!confirm(confirmMsg)) return;

  // Optimistic UI update
  applyEmailMasterUI(newState);
  btn.disabled = true;
  const lbl = document.getElementById('emailMasterLabel');
  lbl.textContent = '⏳ Updating all users…';
  ...
  try {
    ...
    // Firestore batch can only handle 500 writes at a time
    const BATCH_LIMIT = 490;
    let count = 0;
    let batch = writeBatch(db);

    for (const userDoc of usersSnap.docs) {
      batch.update(doc(db, 'users', userDoc.id), { subscribed: newState });
      count++;
    ...
    // Save global setting too
    await setDoc(doc(db, 'settings', 'email'), { enabled: newState }, { merge: true });

    applyEmailMasterUI(newState);
    toast(newState
      ? `📧 ${total} users subscribed!`
      : `🔕 ${total} users unsubscribed!`);

    // Reload users table to show updated toggles
    loadUsers();
  } catch(e) {
    console.error('Error bulk-updating subscriptions:', e);
    applyEmailMasterUI(isCurrentlyOn); // revert on error
    toast('❌ Failed to update subscriptions');
  } finally {
    btn.disabled = false;
  }
};
...
async function loadDashboard() {
  try {
    ...
    // Load email broadcast status
    loadEmailBroadcastStatus();
...
async function loadUsers() {
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      const isSubscribed = u.subscribed !== false; // default true if field missing
      ...
      tr.innerHTML = `
        ...
        <td>${u.posts || 0}</td>
        <td>
          <label class="toggle-switch" title="Toggle email subscription">
            <input type="checkbox" ${isSubscribed ? 'checked' : ''}
              onchange="toggleSubscription('${d.id}', this.checked)">
            <span class="toggle-track"></span>
            <span class="toggle-label">${isSubscribed ? 'On' : 'Off'}</span>
          </label>
        </td>
        <td>
    ...
    table.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 40px; color: #ef4444;">Access Error: Failed to fetch users database.</td></tr>'; 
  }
}

window.toggleSubscription = async (uid, subscribed) => {
  try {
    await updateDoc(doc(db, 'users', uid), { subscribed });
    // Update the label text inline without a full reload
    toast(subscribed ? '✅ User subscribed' : '🔕 User unsubscribed');
    // Update label text next to the toggle that was just changed
    const allToggles = document.querySelectorAll('#usersTable tbody tr');
    allToggles.forEach(row => {
      const cb = row.querySelector('input[type="checkbox"][onchange*="' + uid + '"]');
      if (cb) {
        const label = cb.closest('.toggle-switch')?.querySelector('.toggle-label');
        if (label) label.textContent = subscribed ? 'On' : 'Off';
      }
    });
  } catch(e) { 
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="action-btn" onclick="openEditPostModal('${d.id}')">Edit</button>
...
window.openEditPostModal = async (postId) => {
  try {
    ...
    if (['hackathon','workshop','cultural','symposium'].includes(p.type) || p.eventData) {
      ...
      evGroup.classList.add('edit-event-group');
...
async function loadReports() {
  ...
  body.innerHTML = '<tr><td colspan="3" style="text-align:center; padding: 40px;"><span class="spinner"></span> Loading reports...</td></tr>';

  try {
    const snap = await getDocs(collection(db, 'reports'));
    body.innerHTML = '';
    if (snap.empty) {
      body.innerHTML = '<tr><td colspan="3" style="text-align:center; padding: 40px; color: var(--muted);">No pending reports! 🎉</td></tr>';
      return;
    }
    snap.forEach(d => {
      const r = d.data();
      ...
      tr.innerHTML = `
        ...
        <td><button class="action-btn btn-delete" onclick="dismissReport('${d.id}')">Dismiss</button></td>
      `;
      body.appendChild(tr);
    });
  } catch(e) { body.innerHTML = '<tr><td colspan="3" style="text-align:center; color: #ef4444;">Error loading reports</td></tr>'; }
}
...
// ── SYSTEM-WIDE MESSAGE PURGE ───────────────────────────────────
window.clearAllMessages = async function() {
  if (!confirm("🚨 NUCLEAR ACTION: This will permanently delete EVERY chat and EVERY message on the entire platform. Proceed?")) return;
  if (!confirm("FINAL WARNING: All user conversation data will be lost forever. Continue?")) return;
  ...
  try {
    ...
    toast(`✅ Successfully purged ${chatsSnap.size} chats and ${totalMessages} messages.`);
  } catch (e) {
    console.error('Error purging system messages:', e);
    toast('❌ Error during purge operation');
  } finally {
    btn.disabled = false;
