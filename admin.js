...
async function loadUsers() {
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      tr.innerHTML = `
        <td><div class="lb-avatar">${u.photoURL ? `<img src="${u.photoURL}">` : u.displayName?.charAt(0) || 'U'}</div></td>
        <td><strong>${u.displayName || 'User'}</strong><br><small>${u.email || ''}</small></td>
  ...
  } catch(e) { table.innerHTML = '<tr><td colspan="5">Access Error</td></tr>'; }
}
